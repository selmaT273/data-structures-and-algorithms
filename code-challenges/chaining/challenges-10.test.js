...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArr = [];
  input.forEach(value => {
    let newNewArr = value.filter(eachValue => (eachValue % 5 === 0 && typeof eachValue === 'number')).map(eachValue => 2 ** eachValue);
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter((eachChar) => 
    (eachChar.gender === 'male' || eachChar.gender === 'female')).map(eachChar => 
    eachChar.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.map(eachChar => {
    return eachChar.name;
  })
  .reduce((findShortestName, currentName) => {
    return findShortestName.length < currentName.length ? findShortestName : currentName;
  });  
};
